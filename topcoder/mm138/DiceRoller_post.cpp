...
const int dx[] = {-MAX_N-1, -MAX_N, -MAX_N+1, +1, +MAX_N+1, +MAX_N, +MAX_N-1, -1};
...
const double VCV[] =  {0, 0, 0.13, 0.45, 0.74};

const double VCVD[] = {VCV[1]-VCV[0], VCV[2]-VCV[1], VCV[3]-VCV[2], VCV[4]-VCV[3]};
...
struct BState {
	...
	void copy(BState &s) {
		...
		memcpy(nb, s.nb, sizeof(char) * (MAX_N)*(N+2));
		memcpy(sums, s.sums, sizeof(sums));
	}
	
	INLINE double hvalue(int p) {
		return (grid[p] >= min_value ? grid[p] : V * 0.25) * VCV[nb[p]];
	}
	
	INLINE double hdvalue(int p) {
		return (grid[p] >= min_value ? grid[p] : V * 0.25) * VCVD[nb[p]];
	}
	...
	void move(int d) {
		...
		if (connected()) {
			if (cc_value) {
				cc_value -= hvalue(np);
				REP(i, 4) if (vs[np+dd[i]] < BIGINT) cc_value -= hdvalue(np+dd[i]);
				
				REP(i, 4) if (start+dd[i]==cur-dd[d]) {
					cc_start = false;
					scc_vs_no++;
					REP(i, 4) if (vs[np+dd[i]] < BIGINT) {
						go(np+dd[i]);
						break;
					}
					REP(i, 4) cc_start |= vs[start+dd[i]] == scc_vs_no || start + dd[i] == np;
					break;
				}
			} else {
				scc_vs_no++;
	...
	INLINE bool connected() {
		int parts = 0;
		bool prev_empty = vs[cur + dx[7]] < BIGINT;
		
		REP(i, 8) {
			bool empty = vs[cur + dx[i]] < BIGINT;
			parts += empty && !prev_empty;
			prev_empty = empty;
		}
		return parts <= 1;
	}
	...
	double eval() {
		double rv = 0;
		if (forced_sides.S) {
			REP(i, 6) rv += sums[i][forced_sides[i]];
		} else {
			REP(i, 6) {
				int bv = -1e9;
				int mv = 0;
		...
		if (cc_value && connected()) {
			value_left = cc_value;
			value_left -= hvalue(cur);
			REP(d, 4) if (vs[cur+dd[d]] < BIGINT) value_left -= hdvalue(cur+dd[d]);

			start_connected = cc_start;
		} else {
			scc_vs_no++;
			REP(d, 4) {
				int np = cur + dd[d];
...
double beam_search(VI starts, double time_mul = 1) {
	int BEAM_WIDTH = min(MAX_BEAM_WIDTH, (int)(1600 * time_mul * BS_MUL / BS_RUNS * 30 * 30 * 30 / N / N / N));
...
int main() {
	...
	if (!USE_TL || elapsed() < MAX_TIME) {
		DB(path_score(bpath));
		forced_sides = VI(6);
		REP(i, 6) forced_sides[i] = last_best_sides[i];
		min_value = V;
		REP(i, 6) min_value = min(min_value, forced_sides[i]);
		
		double av = beam_search(VI{bpath[0]}, BS_RUNS);
		DB(av);
		if (av > bv) {
			bpath = last_path;
			bv = av;
	...
	cerr << "[DATA] states_no = " << states_no << endl;
